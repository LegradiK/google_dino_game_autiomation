...
def get_proximity_for_elapsed(elapsed):
    ...
    current_prox = list(default_proximity)
...
def is_game_over():
    """ detect if dino stopped moving for 0.3 seconds """
    global estimated_speed, start_time
    if time.time() - start_time < 4:
        return False
    return estimated_speed < 0.3

def reset_game():
    """ reset all the numbers to start the game from scratch """
    global detection_box, baseline_edges, prev_frame, estimated_speed, alerted, start_time
    ...
    print("Reset complete. Press SPACE if you want to play again.")
...
while dino_alive:
    ...
    # checking time for testing purposes
    for milestone in time_set:
        if elapsed >= milestone and milestone not in alerted:
            alerted.add(milestone)
            print(f"[t={milestone}s] Milestone reached.")

    # change detection_box location as game progresses
    current_prox = get_proximity_for_elapsed(elapsed)
    new_box = get_detection_box_from_proximity(best_region, current_prox)

    if new_box != detection_box and elapsed > 15:
    ...
    # speed and dynamic threshold to accomodate the changes of game speed
